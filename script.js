...
function saveChoices(choices) {
    const userId = "exampleUser"; // 실제 사용자 ID로 변경할 수 있습니다.
    const userChoicesRef = ref(database, 'choices/' + userId);
    set(userChoicesRef, choices)
        .then(() => {
            console.log("선택한 내용이 Firebase에 저장되었습니다.");
        })
        .catch((error) => {
